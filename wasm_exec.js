...
(() => {
    if (typeof process !== undefined) {
        console.log("Scheduled benchmark for 2 seconds from now.");
        setTimeout(benchmark, 2000);
    }
})();

function call_addOne(a) {
    return new Promise(function(resolve, reject) {
        global.addOne(a, function(result, nanos) {
            resolve({result, nanos});
        });
    });
}
...
async function benchmark() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        const result = await call_addOne(i);
        const dt = performance.now() - t0;
        jsTimes.push(dt);
        goTimes.push(result.nanos/(1000*1000));
    }
    report("JS Latency (ms)", jsTimes);
    report("Go Latency (ms)", goTimes);
}
